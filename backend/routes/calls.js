...
// Get active call in a voice channel
router.get('/voice-channel/:channelId/active', [
  ...
  try {
    ...
    if (!isServerMember(channel.server, req.user.id)) {
    ...
    const populatedCall = await Call.findById(call._id)
      .populate('participants.user', 'id username avatar')
      .populate('initiator', 'id username avatar')
      .populate('voiceChannel', 'name server');
    
    console.log('GET /active participants:', populatedCall.participants.map(p => ({
      userId: p.user?.id || p.user?._id,
      username: p.user?.username || 'Abhi(You)',
      leftAt: p.leftAt
    })));
